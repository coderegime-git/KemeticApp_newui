...
(function ($) {
  ...
  function handleQuizHtml(quiz) {
    ...
    if (quiz.has_expired) {
      ...
      if (quiz.can_try) {
        otherHtml += `<a href="/panel/quizzes/${quiz.id}/start" target="_blank" class="btn btn-sm" style="background:#F2C94C; color:#1C1C1C;">${quizPageLang}</a>`;
        // otherHtml += "\n                <a href=\"/panel/quizzes/".concat(quiz.id, "/start\" target=\"_blank\" class=\"btn btn-primary btn-sm mt-15\">").concat(quizPageLang, "</a>\n            ");
      } else {
        otherHtml += `<button type="button" disabled class="btn btn-sm" style="background:#333; color:#F2C94C;">${quizPageLang}</button>`;
