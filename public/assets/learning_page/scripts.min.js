...
(function ($) {
        ...
        <div class="kemetic-note-box js-personal-notes-form" 
            ...
            <div class="note-attachment">
                ...
                <div class="note-attach-box">
                    <button type="button" style="background:white;" class="attach-btn panel-file-manager" 
            ...
            <div class="note-actions">
                <button class="note-btn-save js-save-personal-note">${saveNoteLang}</button>

                ${item.personalNote?.note ? 
                `<button class="note-btn-clear js-clear-personal-note">${clearNoteLang}</button>` 
                : ''}
            </div>
        </div>`;
  ...
  function handleQuizHtml(quiz) {
    ...
    if (quiz.has_expired) {
      ...
      if (quiz.can_try) {
        otherHtml += `<a href="/panel/quizzes/${quiz.id}/start" target="_blank" class="kemetic-btn-gold mt-15 mx-auto d-block" style="width: fit-content; text-align: center;margin-top: 10px;">${quizPageLang}</a>`;
      } else {
        otherHtml += `<button type="button" disabled class="kemetic-show-btn-disabled mt-15 mx-auto d-block" style="width: fit-content; text-align: center;margin-top: 10px;">${quizPageLang}</button>`;
      }
    }
    var html = handleContentBoxHtml(title, hint, img, otherHtml);
  ...
  function handleLiveSessionFinishedHtml(session) {
    ...
    var otherHtml = `
        <a href="${courseUrl}" class="kemetic-btn-outline mt-15">
            ${coursePageLang}
        </a>
    `;
  ...
  function handleLiveSessionNotStartedHtml(session) {
    ...
    var otherHtml = `
        <div class="kemetic-flex mt-15">
            ...
            <a href="${courseUrl}" class="kemetic-btn-outline ml-10">
                ${coursePageLang}
            </a>
        </div>
    `;
  ...
  function handleLiveSessionHtml(session) {
    ...
    if (session.password) {
          otherHtml += `
              <div class="kemetic-info-text mt-5">
                  ${passwordLang}: ${session.password}
              </div>
          `;
      }
      ...
      otherHtml += `
          <div class="kemetic-flex mt-15">
              ...
              <a href="${courseUrl}" 
                class="kemetic-btn-outline ml-10">
                ${coursePageLang}
              </a>
          </div>
      `;
  ...
  function handleFileHtml(file) {
    if (file.online_viewer && file.online_viewer !== '0' || file.downloadable && file.downloadable !== '0') {
      if (file.online_viewer && file.online_viewer !== '0') {
        ...
        var html = `
            <div class="kemetic-file-viewer-wrapper">

                <iframe 
                    src="/ViewerJS/index.html#${file.file_path}" 
                    class="kemetic-file-iframe ${file.downloadable && file.downloadable !== '0' ? 'has-download-card' : ''}" 
                    frameborder="0" 
                    allowfullscreen>
                </iframe>
        ...
        if (file.downloadable && file.downloadable !== '0') {
         html += `
          <div class="kemetic-download-box">
              <span class="file-title">${downloadTheFileLang}</span>
              <a href="${courseUrl}/file/${file.id}/download" 
                class="kemetic-download-btn" target="_blank">
                ${downloadLang}
              </a>
          </div>`;
        ...
        if (file.is_video) {
          ...
          var $html = `
                <div class="js-video-and-download kemetic-video-wrapper">
            ...
            if (file.downloadable && file.downloadable !== '0') {
                $html += `
                    <div class="kemetic-download-box">
                        <span class="download-title">${downloadTheFileLang}</span>
                        <a href="${courseUrl}/file/${file.id}/download" 
                          class="kemetic-download-btn" 
                          target="_blank">
                            ${downloadLang}
                        </a>
                    </div>
      ...
      switch (file.storage) {
        ...
        case 's3':
          ...
          var _$html = `
            <div class="kemetic-video-wrapper w-100">
                <div class="js-other-content-video-player learning-content-video-player"></div>
            </div>`;
        ...
        case 'secure_host':
          ...
          var $secureHosthtml = `
            <div class="kemetic-video-wrapper w-100">
                <div class="js-secure-host-video-player learning-content-video-player"></div>
            </div>`;
        ...
        case 'upload_archive':
          ...
          var _otherHtml = `
              <a href="${courseUrl}/file/${file.id}/showHtml" 
                target="_blank" 
                class="kemetic-btn kemetic-btn-gold mt-15">
                ${showLang}
              </a>
          `;
  ...
  function handleTextLessonHtml(textLesson) {
    ...
    var html = `
        <div class="kemetic-text-lesson p-15 p-lg-30">

            <h4 class="kemetic-title">${textLesson.title}</h4>

            ${textLesson.image ? `
                <div class="kemetic-main-image mt-15">
                    <img src="${textLesson.image}" alt="${textLesson.title}" >
                </div>
            ` : ''}

            <div class="kemetic-content">
                ${textLesson.content}
            </div>
    ...
    if (textLesson.attachments && Object.keys(textLesson.attachments).length) {
      ...
      html += `
        <div class="kemetic-attachment-box mt-20">

            <h3 class="kemetic-section-title">${attachmentsLang}</h3>
      ...
      Object.keys(textLesson.attachments).forEach(function (key) {
        ...
        html += `
          <div class="col-12 col-lg-3 mt-10 mt-lg-0">
              <a href="${courseUrl}/file/${attachment.file.id}/download" 
                class="kemetic-attachment-item">

                  <span class="kemetic-attachment-icon">
                      <i data-feather="download-cloud" width="16" height="16"></i>
                  </span>

                  <div>
                      <span class="kemetic-attachment-title">${attachment.file.title}</span>
                      <span class="kemetic-attachment-meta">${attachment.file.file_type} | ${attachment.file.volume}</span>
                  </div>
              </a>
          </div>
          `;
  ...
  function addContentLoading() {
    // var html = "<div class=\"learning-content-loading d-flex align-items-center justify-content-center flex-column w-100 h-100\">\n            <img src=\"/assets/default/img/loading.gif\" alt=\"\">\n            <p class=\"mt-10\">".concat(pleaseWaitForTheContentLang, "</p>\n        </div>");
      var html = `<div class="learning-content-loading d-flex align-items-center justify-content-center flex-column w-100 h-100">
                        <img src="/assets/default/img/loading.gif" alt="">
                        <p class="mt-10" style="color:#F2C94C;">${pleaseWaitForTheContentLang}</p>
                    </div>`;
    learningPageContent.html(html);
  }
  ...
  function contentEmptyStateHtml() {
    ...
    var html = `
        <div class="kemetic-empty-box d-flex align-items-center justify-content-center flex-column">
            ...
            <div class="d-flex align-items-center flex-column mt-10 text-center">
                <h3 class="kemetic-empty-title">${learningPageEmptyContentTitleLang}</h3>
                <p class="kemetic-empty-hint mt-5">${learningPageEmptyContentHintLang}</p>
            </div>
  ...
  $('body').on('click', '.js-save-history-message', function () {
    ...
    $.post(action, data, function (result) {
      if (result && result.code === 200) {
        Swal.fire({
          icon: 'success',
          html: '<h3 class="font-20 text-center text-dark">' + sendAssignmentSuccessLang + '</h3>',
          showConfirmButton: false
        });
        setTimeout(function () {
          window.location.reload();
  ...
  $('body').on('click', '.js-save-history-rate', function () {
    ...
    $.post(action, data, function (result) {
      if (result && result.code === 200) {
        Swal.fire({
          icon: 'success',
          html: '<h3 class="font-20 text-center text-dark">' + saveAssignmentRateSuccessLang + '</h3>',
          showConfirmButton: false
        });
        setTimeout(function () {
          window.location.reload();
