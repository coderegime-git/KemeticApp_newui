...
(function ($) {
  ...
  $('body').off('click', '.save-question').on('click', '.save-question', function (e) {
    e.preventDefault();
    var $this = $(this);
    ...
    form.find('.js-frontend-invalid').remove();
    ...
    function showError(element, msg) {
      ...
      var feedback = parent.find('.invalid-feedback');
      if (feedback.length) {
        feedback.text(msg).show();
      } else {
        element.after('<div class="invalid-feedback d-block js-frontend-invalid">' + msg + '</div>');
      }
      hasError = true;
    }

    var title = form.find('.js-ajax-title');
    if (title.length && !title.val().trim()) {
      showError(title, 'Please fill the required field');
    }

    var grade = form.find('.js-ajax-grade');
    if (grade.length && !grade.val().trim()) {
      showError(grade, 'Please fill the required field');
    }

    var correct = form.find('.js-ajax-correct');
    if (correct.length && !correct.val().trim()) {
      showError(correct, 'Please fill the required field');
    }

    // Multiple choice answer titles
    var answerTitles = form.find('.js-ajax-answer-title');
    if (answerTitles.length > 0) {
      answerTitles.each(function() {
        if (!$(this).val().trim()) {
          showError($(this), 'Please fill the required field');
        }
      });
    }
