...
(function ($) {
  ...
  // Fix: quiz question/answer image upload buttons inside SweetAlert modals
  // NOTE: We bypass $.fn.filemanager() because that plugin wraps window.open() inside
  // a this.on('click') — when called from a delegated handler, the popup would
  // only open on the SECOND click (first click just binds the inner handler).
  // Instead, we call window.open() directly so the first click always works.
  function lfmOpen(btn, type) {
    var $btn         = $(btn);
    var targetInput  = $('#' + $btn.data('input'));
    var targetPreview = $('#' + $btn.data('preview'));
    window.open('/laravel-filemanager?type=' + type, 'FileManager', 'width=900,height=600');
    window.SetUrl = function (items) {
      var filePath = items.map(function (i) { return i.url; }).join(',');
      targetInput.val('').val(filePath).trigger('change');
      if (targetPreview.length) {
        targetPreview.html('');
        items.forEach(function (item) {
          targetPreview.append($('<img>').css('height', '5rem').attr('src', item.thumb_url));
        });
        targetPreview.trigger('change');
      }
    };
  }
  ...
  $(document).ready(function () {
    ...
    if (typeof SimpleBar !== 'undefined' && document.getElementById('panel-sidebar-scroll')) {
