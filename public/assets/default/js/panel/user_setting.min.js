...
(function ($) {
  ...
  function validateSettingsForm() {
      // Check if we are on the financial step/tab
      var isFinancialTab = $('#identity-content').hasClass('settings-active') || 
                           $('input[name="step"]').val() == '6' || 
                           $('input[name="step"]').val() == '7';
      ...
      var isValid = true;
      var $bankSelect = $('select[name="bank_id"]');
      
      // If bank select exists, we are on financial step
      if ($bankSelect.length > 0) {
          // Check bank type select
          if (!$bankSelect.val()) {
              isValid = false;
          ...
          // Check dynamically generated bank specifications
          $('.js-bank-specifications-card input').each(function() {
              if (!$(this).val().trim()) {
                  isValid = false;
  ...
  $('body').on('change', '.js-user-bank-input', function (e) {
    ...
    if (specifications) {
      Object.entries(JSON.parse(specifications)).forEach(function (_ref, key) {
        ...
        var isAccountNum = item.toLowerCase().indexOf('account number') !== -1 || item.toLowerCase().indexOf('iban') !== -1;
        var extraAttr = isAccountNum ? ' maxlength="35" oninput="this.value = this.value.replace(/[^a-zA-Z0-9]/g, \'\')"' : '';
        html += '<div class="form-group">\n' + '         <label class="font-weight-500 text-dark-blue">' + item + ' <span class="text-danger">*</span></label>\n' + '         <input type="text" name="bank_specifications[' + index + ']" value="" class="form-control" ' + extraAttr + '/>\n' + ' </div>';
      });
    }
