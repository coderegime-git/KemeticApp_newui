    ...
    private function handleDaysAndTimeFilter($query, Request $request)
    {
        ...
        if (isset($minTime) and isset($maxTime)) {
            ...
            if (!empty($meetingsTimes)) {
                ...
                foreach ($meetingsTimes as $meetingsTime) {
                    // Normalize the time format to ensure space before AM/PM
                    $normalizedTime = preg_replace('/\s*([AP]M)$/i', ' $1', $meetingsTime['time']); // Add a single space before AM/PM
                
                    // Split start and end times
                    $timeParts = explode('-', $normalizedTime);
                
                    // Check if both time parts exist before parsing
                    if (count($timeParts) === 2) {
                        ...
                        try {
                            ...
                            // Handle parse errors, maybe log or skip this time entry
                            continue;
                        }
