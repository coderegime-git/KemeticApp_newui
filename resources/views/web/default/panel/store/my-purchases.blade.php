...
@push('styles_top')
    ...
    <style>
        ...
        .cj-pill.transit {
            background: rgba(42, 122, 78, .22);
            color: #6fffa0;
            border: 1px solid rgba(42, 122, 78, .4);
        }
        ...
        .cj-pill.error {
            ...
            color: #ff9999;
            border: 1px solid rgba(122, 31, 31, .4);
        }
        ...
        .cj-cell:nth-child(2n) {
            border-right: none;
        }

        .cj-cell:nth-last-child(-n+2) {
            border-bottom: none;
        }
        ...
        .cj-routes-body.open::-webkit-scrollbar {
            width: 4px;
        }

        .cj-routes-body.open::-webkit-scrollbar-track {
            background: transparent;
        }
        ...
        .cj-step:last-child .cj-step-connector {
            display: none;
        }
        ...
        .cj-step:last-child .cj-step-content {
            padding-bottom: 0;
        }
                ...
                <div class="row text-center">
                    ...
                    <div class="col-6 col-md-3 mb-4">
                        <div class="d-flex flex-column align-items-center">
                            ...
                            <strong
                                class="text-dark-blue mt-2">{{ !empty($totalPurchase) ? handlePrice($totalPurchase) : 0 }}</strong>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="seller_id" class="form-control select2">
                                            ...
                                            @foreach($sellers as $seller)
                                                <option value="{{ $seller->id }}" @if(request()->get('seller_id') == $seller->id)
                                                selected @endif>{{ $seller->full_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="type" class="form-control">
                                            ...
                                            @foreach(\App\Models\Product::$productTypes as $productType)
                                                <option value="{{ $productType }}" @if(request()->get('type') == $productType)
                                                selected @endif>{{ trans('update.product_type_' . $productType) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <select name="status" class="form-control">
                                            ...
                                            @foreach(\App\Models\ProductOrder::$status as $orderStatus)
                                                @if($orderStatus != 'pending')
                                                    <option value="{{ $orderStatus }}" @if(request()->get('status') == $orderStatus)
                                                    selected @endif>{{ trans('update.product_order_status_' . $orderStatus) }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                ...
                                @foreach($orders as $order)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left" data-label="{{ trans('update.order_id') }}">
                                            ...
                                            <span class="d-block font-12 text-gray">{{ $order->quantity }}
                                                {{ trans('update.product') }}</span>
                                            @if($isCJ && $isWaiting)
                                                <button class="btn-cj-track mt-5" onclick="cjOpenModal({{ $order->id }})"
                                        ...
                                        <td data-label="{{ trans('public.discount') }}">@if(!empty($order->sale->discount))
                                        {{ handlePrice($order->sale->discount) }} @else - @endif</td>
                                        <td data-label="{{ trans('cart.tax') }}">@if(!empty($order->sale->tax))
                                        {{ handlePrice($order->sale->tax) }} @else - @endif</td>
                                        <td data-label="{{ trans('update.delivery_fee') }}">
                                            @if(!empty($order->sale->product_delivery_fee))
                                            {{ handlePrice($order->sale->product_delivery_fee) }} @else - @endif</td>
                                        <td data-label="{{ trans('financial.total_amount') }}">
                                            {{ handlePrice($order->sale->total_amount) }}</td>
                                        ...
                                        <td data-label="{{ trans('public.status') }}">
                                            @if($order->status == \App\Models\ProductOrder::$waitingDelivery)
                                                <span
                                                    class="text-warning">{{ trans('update.product_order_status_waiting_delivery') }}</span>
                                            @elseif($order->status == \App\Models\ProductOrder::$success)
                                                <span
                                                    class="text-dark-blue status-success">{{ trans('update.product_order_status_success') }}</span>
                                        ...
                                        <td data-label="{{ trans('public.date') }}">
                                            {{ dateTimeFormat($order->created_at, 'j M Y H:i') }}</td>
                                        <td>
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu">
                                                    @if($order->product && $order->product->isVirtual())
                                                        <a href="/panel/store/products/{{ $order->product->id }}/getFilesModal"
                                                            class="dropdown-item">{{ trans('home.download') }}</a>
                                                    @endif
                                                    <a href="/panel/store/purchases/{{ $order->sale_id }}/productOrder/{{ $order->id }}/invoice"
                                                        target="_blank" class="dropdown-item">{{ trans('public.invoice') }}</a>
                                                    ...
                                                    @if($order->status == \App\Models\ProductOrder::$shipped)
                                                        <button type="button" data-sale-id="{{ $order->sale_id }}"
                                                            data-product-order-id="{{ $order->id }}"
                                                            class="js-view-tracking-code dropdown-item">{{ trans('update.view_tracking_code') }}</button>
                                                        <button type="button" data-sale-id="{{ $order->sale_id }}"
                                                            data-product-order-id="{{ $order->id }}"
                                                            class="js-got-the-parcel dropdown-item">{{ trans('update.i_got_the_parcel') }}</button>
                                                    @endif

                                                    @if($isCJ && ($isWaiting || $order->status == \App\Models\ProductOrder::$shipped))
                                                        <button type="button" class="dropdown-item"
                                                            onclick="cjOpenModal({{ $order->id }})">
                                                            <svg style="width:13px;height:13px;margin-right:6px;vertical-align:-1px"
...
@push('scripts_bottom')
    <script>
        ...
        $(document).off('click', 'a[href*="getFilesModal"]').on('click', 'a[href*="getFilesModal"]', function (e) {
            e.preventDefault();
