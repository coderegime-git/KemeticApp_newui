        ...
        <div class="activities-container mt-25 p-20 p-lg-35 d-flex flex-wrap justify-content-between">
            ...
            <div class="col-6 col-md-3 mb-4">
                <div class="d-flex flex-column align-items-center">
                    ...
                    <strong class="font-30 font-weight-bold mt-2 text-dark-blue">{{ !empty($totalPurchase) ? handlePrice($totalPurchase) : 0 }}</strong>
                                ...
                                <tr>
                                    <td class="text-left">
                                        <div class="user-inline-avatar d-flex align-items-center">
                                            ...
                                            <div class="ml-3">
                                                <span class="d-block">{{ $order->seller->full_name ?? '' }}</span>
                                                <span class="d-block font-12 text-gray">{{ $order->seller->email ?? '' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-left">
                                    ...
                                    <td>@if(!empty($order->sale->discount)) {{ handlePrice($order->sale->discount) }} @else - @endif</td>
                                    <td>@if(!empty($order->sale->tax)) {{ handlePrice($order->sale->tax) }} @else - @endif</td>
                                    <td>@if(!empty($order->sale->product_delivery_fee)) {{ handlePrice($order->sale->product_delivery_fee) }} @else - @endif</td>
                                        ...
                                        <div class="btn-group dropdown table-actions">
                                            ...
                                            <div class="dropdown-menu">
                                                @if($order->product && $order->product->isVirtual())
                                                    <a href="/panel/store/products/{{ $order->product->id }}/getFilesModal" class="d-block mt-10">{{ trans('home.download') }}</a>
                                                @endif
                                                <a href="/panel/store/purchases/{{ $order->sale_id }}/productOrder/{{ $order->id }}/invoice" target="_blank" class="d-block mt-10">{{ trans('public.invoice') }}</a>
                                                @if($order->product && $order->status == \App\Models\ProductOrder::$success)
                                                    <a href="{{ $order->product->getUrl() }}" target="_blank" class="d-block mt-10">{{ trans('public.feedback') }}</a>
                                                @endif
                                                @if($order->status == \App\Models\ProductOrder::$shipped)
                                                    <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-view-tracking-code d-block mt-10">{{ trans('update.view_tracking_code') }}</button>
                                                    <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-got-the-parcel d-block mt-10">{{ trans('update.i_got_the_parcel') }}</button>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
