...
<li data-id="{{ !empty($faq) ? $faq->id : '' }}" class="accordion-row k-accordion-item">
    <div class="k-accordion-header" data-toggle="collapse" href="#collapseFaq{{ !empty($faq) ? $faq->id : 'record' }}"
    ...
    <div id="collapseFaq{{ !empty($faq) ? $faq->id : 'record' }}"
        class="collapse @if(empty($faq)) show @endif k-accordion-body"
        aria-labelledby="faq_{{ !empty($faq) ? $faq->id : 'record' }}">
        <div class="js-content-form faq-form"
            data-action="/panel/store/products/faqs/{{ !empty($faq) ? $faq->id . '/update' : 'store' }}">
            <input type="hidden" name="ajax[{{ !empty($faq) ? $faq->id : 'new' }}][product_id]"
            ...
            <div class="row">
                <div class="col-12 col-lg-6">
                    @if(!empty(getGeneralSettings('content_translate')))
                        <div class="form-group">
                            ...
                            <select name="ajax[{{ !empty($faq) ? $faq->id : 'new' }}][locale]"
                                ...
                                data-id="{{ !empty($faq) ? $faq->id : '' }}" data-relation="faqs"
                    ...
                    @else
                        <input type="hidden" name="ajax[{{ !empty($faq) ? $faq->id : 'new' }}][locale]"
                            value="{{ $defaultLocale }}">
                    @endif

                    <div class="form-group">
                        <label class="input-label">{{ trans('public.title') }} <span
                    ...
                    <div class="form-group">
                        <label class="input-label">{{ trans('public.answer') }} <span
                                class="text-danger">*</span></label>
                        <textarea name="ajax[{{ !empty($faq) ? $faq->id : 'new' }}][answer]"
                            class="js-ajax-answer form-control"
