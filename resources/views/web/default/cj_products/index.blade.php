...
<div class="shop-wrap">
  ...
  <div class="shop-trend">
    ...
    <article class="shop-card">
      <div class="shop-media">
        <a href="{{ route('cj.products.show', $product['pid']) }}" class="image-box__a">
          <img src="{{ $product['productImage'] ?? '' }}" class="img-cover" alt="{{ $product['productNameEn'] ?? '' }}" loading="lazy">
        </a>
        ...
        <div class="shop-meta">
          ...
          <a href="{{ route('cj.products.show', $product['pid']) }}" class="shop-atk" style="display:inline-block;text-align:center">View</a>
        </div>
      </div>
    </article>
    @endforeach
  ...
  {{-- ── Search ──────────────────────────────────────────────────── --}}
  <form action="{{ route('cj.products.index') }}" method="GET">
    <div class="shop-search">
      <span style="color:var(--gold);font-weight:900">🔎</span>
      <input type="hidden" name="category_id" value="{{ $selectedCategoryId }}">
      <input type="text" name="search" class="form-control border-0"
             value="{{ request('search') }}" placeholder="Search CJ products…">
  ...
  {{-- ── Filters ─────────────────────────────────────────────────── --}}
  <form action="{{ route('cj.products.index') }}" method="GET" id="cjFiltersForm">
    <input type="hidden" name="search" value="{{ request('search') }}">

    {{-- Category Chips --}}
    @if(!empty($categories))
    <div class="shop-chips">
      <a href="{{ route('cj.products.index') }}{{ request('search') ? '?search='.request('search') : '' }}">
        ...
        @if(!empty($firstLevel['categoryFirstList']))
          @foreach($firstLevel['categoryFirstList'] as $secondLevel)
            @if(!empty($secondLevel['categorySecondList']))
              ...
              <a href="{{ route('cj.products.index', ['category_id' => $thirdLevel['categoryId'], 'search' => request('search')]) }}">
              ...
              @endforeach
            @else
            <a href="{{ route('cj.products.index', ['category_id' => $secondLevel['categorySecondId'] ?? '', 'search' => request('search')]) }}">
    ...
    <div class="cj-filter-bar" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:16px 0">
      ...
      <a href="{{ route('cj.products.index') }}" class="cj-clear-btn">✕ Clear</a>
      @endif
    </div>
    ...
    <section class="shop-grid">
      @if(!empty($products))
        ...
        <article class="shop-p">
          ...
          {{-- Image --}}
          <div class="shop-img">
            <a href="{{ route('cj.products.show', $product['pid']) }}">
          ...
          <div class="shop-pd">
            ...
            {{-- Category --}}
            @if(!empty($product['categoryName']))
            <div style="font-size:11px;opacity:.55;margin-bottom:4px">{{ Str::limit($product['categoryName'], 35, '..') }}</div>
            @endif
            ...
            <div class="shop-row-end" style="margin-top:6px">
              <div class="shop-price-row">
                <span class="real" style="font-weight:900">${{ number_format((float)($product['sellPrice'] ?? 0), 2) }}</span>
              </div>
              <a href="{{ route('cj.products.show', $product['pid']) }}" class="shop-atk">View</a>
            </div>
          </div>
...
.cj-page-btn:hover, .cj-page-btn.active {
