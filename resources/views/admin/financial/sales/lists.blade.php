                                        ...
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                <div class="media-body">
                                                    ...
                                                    @if(!empty($sale->item_id) && $sale->item_id !== '---')
                                            ...
                                            <td>
                                                <span class="font-weight-bold">
                                                    @if($sale->type == \App\Models\Sale::$book)
                                                        <span class="badge badge-info">
                                                            <i class="fas fa-book mr-1"></i>Book
                                                        </span>
                                                    @elseif($sale->type == \App\Models\Sale::$registrationPackage)
                                                        {{ trans('update.registration_package') }}
                                                    @elseif($sale->type == \App\Models\Sale::$product)
                                                        <span class="badge badge-secondary">
                                                            <i class="fas fa-box mr-1"></i>{{ trans('update.product') }}
                                                        </span>
                                            ...
                                            {{-- Actions --}}
                                            <td>
                                                {{-- Book Sale: View detail --}}
                                                @if($sale->type == \App\Models\Sale::$book)
                                                    @can('admin_sales_list')
                                                        <a href="{{ getAdminPanelUrl() }}/financial/sales/{{ $sale->id }}/view-book"
                                                           target="_blank"
                                                           title="View Book Sale Detail"
                                                           class="btn btn-sm btn-outline-info py-0 px-1">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                    @endcan

                                                {{-- Product Sale: View detail --}}
                                                @elseif($sale->type == \App\Models\Sale::$product)
                                                    @can('admin_sales_list')
                                                        <a href="{{ getAdminPanelUrl() }}/financial/sales/{{ $sale->id }}/view-product"
                                                           target="_blank"
                                                           title="View Product Sale Detail"
                                                           class="btn btn-sm btn-outline-secondary py-0 px-1">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                    @endcan

                                                {{-- Webinar / Bundle: invoice --}}
                                                @elseif(!empty($sale->webinar_id) or !empty($sale->bundle_id))
                                                    @can('admin_sales_invoice')
                                                        <a href="{{ getAdminPanelUrl() }}/financial/sales/{{ $sale->id }}/invoice"
                                                           target="_blank"
                                                           title="{{ trans('admin/main.invoice') }}">
                                                            <i class="fa fa-print" aria-hidden="true"></i>
                                                        </a>
                                                    @endcan
                                                @endif

                                                {{-- Refund button (all types except subscribe) --}}
                                                @can('admin_sales_refund')
                                                    @if(empty($sale->refund_at) && $sale->payment_method != \App\Models\Sale::$subscribe)
