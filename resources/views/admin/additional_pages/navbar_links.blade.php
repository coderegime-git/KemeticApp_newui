                            ...
                            <div class="row">
                                <div class="col-12 col-md-8 col-lg-6">
                                    <form action="{{ getAdminPanelUrl() }}/additional_page/navbar_links/store" method="post">
                                        ...
                                        <div class="form-group">
                                            <label>{{ trans('admin/main.icons') }}</label>
                                            <div class="input-group">
                                                <div class="input-group-prepend">
                                                    <button type="button" class="input-group-text admin-file-manager" data-input="image_icon" data-preview="holder">
                                                        <i class="fa fa-upload"></i>
                                                    </button>
                                                </div>
                                                <input type="text" name="value[icons]" id="image_icon" value="{{ !empty($navbar_link) ? $navbar_link->icons : old('value.icons') }}" class="form-control @error('value.icons')  is-invalid @enderror"/>
                                                <div class="input-group-append">
                                                    <button type="button" class="input-group-text admin-file-view" data-input="image_icon">
                                                        <i class="fa fa-eye"></i>
                                                    </button>
                                                </div>
                                                @error('value.icons')
                                    ...
                                    @if(!empty($items))
                                        @foreach($items as $key => $val)
                                            <tr>
                                                <td>{{ $val['title'] }}</td>
                                                <td>{{ $val['link'] }}</td>
                                                <td><img src="{{ url($val['icons']) }}" alt="icon" width="40"></td>
                                                <td>{{ $val['order'] }}</td>
                                                <td>
