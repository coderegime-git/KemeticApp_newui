...
window.makeVideoPlayerHtml = function (path, storage, height, tagId) {
    ...
    if (storage === 'youtube' || storage === 'vimeo') {
        html = '<video id="' + tagId + '" class="video-js vjs-16-9" width="100%" height="' + height + '"></video>';
        ...
        html = '<video id="' + tagId + '" oncontextmenu="return false;" controlsList="nodownload" class="video-js vjs-16-9" controls preload="auto" width="100%" height="' + height + '"><source src="' + path + '" type="video/mp4"/></video>';
    }
